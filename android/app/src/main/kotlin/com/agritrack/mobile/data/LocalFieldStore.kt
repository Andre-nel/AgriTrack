...
class LocalFieldStore(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, DB_VERSION) {
    ...
    fun saveSnapshot(snapshot: FarmSnapshot, makeActive: Boolean = true) {
        ...
        try {
            ...
            saveArray(snapshot.farm.id, "gate", json.optJSONArray("gates"), "id", now)
            saveArray(snapshot.farm.id, "fence_section", json.optJSONArray("fence_sections"), "id", now)
            saveArray(snapshot.farm.id, "fence_event", json.optJSONArray("fence_events"), "id", now)
            saveArray(snapshot.farm.id, "shearer", json.optJSONArray("shearers"), "id", now)
            saveArray(snapshot.farm.id, "shearing_bale_code", json.optJSONArray("shearing_bale_codes"), "id", now)
            saveArray(snapshot.farm.id, "shearing_session", json.optJSONArray("shearing_sessions"), "id", now)
